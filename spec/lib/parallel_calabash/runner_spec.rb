...
describe ParallelAppium::Runner do
  ...
  describe :execute_command_for_process do
    it 'should execute the command with correct env variables set and return exit status 0 when command gets executed successfully' do
      ParallelAppium::AdbHelper.should_receive(:device_for_process).with(3).and_return("DEVICE3")
      expect(ParallelAppium::Runner.execute_command_for_process(3,'echo $ADB_DEVICE_ARG;echo $TEST_PROCESS_NUMBER',true)).to eq ({:stdout=>"DEVICE3\n4\n", :exit_status=>0})
    end

    it 'should return exit status of 1' do
      expect(ParallelAppium::Runner.execute_command_for_process(3,"ruby -e 'exit(1)'",true)).to eq ({:stdout=>"", :exit_status=>1})
    end
  end
