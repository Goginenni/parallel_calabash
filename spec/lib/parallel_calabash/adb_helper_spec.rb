...
describe ParallelAppium::AdbHelper do
  ...
  describe :device_id_and_model do
    it 'should not match any devices in list of devices attached line' do
      expect(ParallelAppium::AdbHelper.new([]).device_id_and_model("List of devices attached")).to eq nil
    end

    it 'should match devices if there is a space after the word device' do
      expect(ParallelAppium::AdbHelper.new([]).device_id_and_model("emulator-5554  device ")).to eq \
         ["emulator-5554", nil]
    end
    ...
    it 'should not match a device if it is an empty line' do
      expect(ParallelAppium::AdbHelper.new([]).device_id_and_model("")).to eq nil
    end

    it 'should match physical devices' do
      output = "192.168.56.101:5555 device product:vbox86p model:device1 device:vbox86p"
      expect(ParallelAppium::AdbHelper.new([]).device_id_and_model(output)).to eq ["192.168.56.101:5555", "device1"]
    end
  end
  ...
  describe :filter_device do
    it 'should return devices if no filter is specified' do
      ...
      expect(ParallelAppium::AdbHelper.new([]).filter_device(device)).to eq device
    end

    it 'should match devices that match the filter' do
      device = ["192.168.56.101:5555", "device1"]
      expect(ParallelAppium::AdbHelper.new(["device1"]).filter_device(device)).to eq device
    end

    it 'should not return devices that do not match the filter' do
      device = ["192.168.56.101:5555", "device1"]
      expect(ParallelAppium::AdbHelper.new(["notmatching"]).filter_device(device)).to eq nil
    end

    it 'can also match on ip address' do
      device = ["192.168.56.101:5555", "device1"]
      expect(ParallelAppium::AdbHelper.new(["192.168.56.101"]).filter_device(device)).to eq device
    end
  end
  ...
  describe :connected_devices_with_model_info do
    ...
    it 'should return device_id, device_name and screenshot_prefix details' do
      adb_helper = ParallelAppium::AdbHelper.new([])
...
describe ParallelAppium::IosHelper do
  ...
  describe :test_remove_unconnected_devices do
    it 'Removes unconnected devices' do
      expect(ParallelAppium::IosHelper.new(nil, nil, {},
                                             "name [udid1]\nname2 [udid2]\nname3 [udid-unknown]")
  ...
  describe :test_compute_simulators do
    it 'allocates ports to users' do
      expect(ParallelAppium::IosHelper.new([], {}, {USERS:['a', 'b'], INIT:'foo'}, '')
                 .compute_simulators)
    ...
    it 'allocates other ports to users' do
      expect(ParallelAppium::IosHelper.new([], {}, {USERS:['a', 'b'], INIT:'foo', CALABASH_SERVER_PORT:100}, '')
                 .compute_simulators)
  ...
  describe :test_compute_devices do
    it 'returns nothing with no users' do
      expect(ParallelAppium::IosHelper.new([], {}, {DEVICES: [{DEVICE_TARGET: 'udid'}]}, 'name [udid]')
                 .compute_devices)
          .to eq []
    end

    it 'fails with no devices' do
      expect { ParallelAppium::IosHelper.new([], {}, {DEVICES: [{DEVICE_TARGET: 'udid'}],
                                                        USERS: ['a']}, 'name [udon\'t]')
                   .compute_devices }
